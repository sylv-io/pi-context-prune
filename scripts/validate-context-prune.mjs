...
function assertSourceOrder() {
  ...
  const guardReturn = indexSource.indexOf('return { ok: false, reason: "below-threshold" };');
  const drain = indexSource.indexOf("pendingBatches.length = 0;");
  const firstFrontierAdvance = indexSource.indexOf("frontier.advance(frontierSnapshot);");

  assert(guardReturn > 0, "below-threshold return is missing");
  assert(drain > guardReturn, "below-threshold skip must return before draining pending batches");
  assert(
  ...
  const initialStatusUpdate = indexSource.indexOf("setPruneStatusWidgetBestEffort(");
  ...
  const sessionIndexAppend = sessionIndexRegion.indexOf("appendEntry(CUSTOM_TYPE_INDEX");
  ...
  const runtimeAddBatch = runtimeSummaryRegion.indexOf("indexer.addBatch(batch, pi)");
  ...
  const sessionPersistIndex = sessionSummaryRegion.indexOf("persistBatchIndex(batch, appendEntry)");
  ...
  assert(
    runtimeFrontierAppend >= 0 && runtimeFrontierAdvance > runtimeFrontierAppend,
  ...
  const sessionFrontierAppend = sessionFrontierRegion.indexOf("appendEntry(CUSTOM_TYPE_FRONTIER");
  const sessionFrontierAdvance = sessionFrontierRegion.indexOf(
    "frontier.advance(frontierSnapshot)",
  );
  assert(
    sessionFrontierAppend >= 0 && sessionFrontierAdvance > sessionFrontierAppend,
  ...
  const runtimeAppend = indexerSource.indexOf("pi.appendEntry(CUSTOM_TYPE_INDEX");
...
function writeHarness() {
  const source = [
    ...
    "",
    "// Guard thresholds count only pruneable raw tokens and tool calls.",
    ...
    "const almostEnoughSmall = evaluatePruneGuard([batch(Array(7).fill(10))], config);",
    "assert.equal(almostEnoughSmall.shouldPrune, false);",
    ...
    "assert.equal(manySmall.estimatedRawTokens, 24);",
    "const disabledCallThreshold = evaluatePruneGuard([batch(Array(8).fill(10))], { ...config, minPruneToolCalls: 0 });",
    "assert.equal(disabledCallThreshold.shouldPrune, false);",
    "const zeroThresholds = evaluatePruneGuard([batch([10])], { ...config, minPruneRawTokens: 0, minPruneToolCalls: 0 });",
    "assert.equal(zeroThresholds.shouldPrune, true);",
    "",
