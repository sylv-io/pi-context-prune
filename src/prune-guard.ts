import type { CapturedBatch, ContextPruneConfig, PreserveToolResultRule } from "./types.js";
...
export function evaluatePruneGuard(
  ...
  const toolCallThresholdReached =
    config.minPruneToolCalls > 0 && eligibleToolCallCount >= config.minPruneToolCalls;
  const shouldPrune = estimatedRawTokens >= config.minPruneRawTokens || toolCallThresholdReached;
